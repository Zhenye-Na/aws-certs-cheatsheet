...
1. Find the root device volume, root device volume will have the snapshotID, "Action" -> "Create Snapshot"
...
references for comparing EBS with EFS:
...
Snowball Edge adds the computational capability to the device, it supports a custom EC2 AMI so you can perform processing on the go
